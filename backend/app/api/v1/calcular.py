"""
Endpoint /api/v1/calcular — única fonte de verdade para cálculos PDA.
...
# Tabelas B.8 e B.9 mantidas aqui para o endpoint ser a fonte única do cálculo.
# Evita divergência com tabelas auxiliares antigas em app.nbr5419.parte2_tabelas.
_UW_PLD = [0.35, 0.5, 1.0, 1.5, 2.5, 4.0, 6.0]
_PLD_TABLE: dict[str, list[float]] = {
    "AEREO_NAO_BLINDADO":      [1, 1, 1,   1,   1,    1,    1],
    "ENTERRADO_NAO_BLINDADO":  [1, 1, 1,   1,   1,    1,    1],
    "BLINDADO_5_20_OHM_KM":    [1, 1, 1,   1,   0.95, 0.9,  0.8],
    "BLINDADO_1_5_OHM_KM":     [1, 1, 0.9, 0.8, 0.6,  0.3,  0.1],
    "BLINDADO_MENOS_1_OHM_KM": [1, 0.85, 0.6, 0.4, 0.2,  0.04, 0.02],
}
_UW_PLI = [1.0, 1.5, 2.5, 4.0, 6.0]
_PLI_TABLE: dict[str, list[float]] = {
    "ENERGIA": [1, 0.6, 0.3, 0.16, 0.1],
    "SINAL":   [1, 0.5, 0.2, 0.08, 0.04],
}

def _nearest_table_value(xs:list[float], ys:list[float], x:float)->float:
...
def _pld(rs:str, uw:float)->float:
    """Tabela B.8 — PLD por condição de blindagem/roteamento e UW."""
    vals = _PLD_TABLE.get(rs)
    if vals is None:
        return 1.0
    return _nearest_table_value(_UW_PLD, vals, uw)

def _pli(tipo_linha:str, uw:float)->float:
    """Tabela B.9 — PLI depende do tipo da linha elétrica (ENERGIA/SINAL) e UW."""
    vals = _PLI_TABLE.get(tipo_linha)
    if vals is None:
        return 1.0
    return _nearest_table_value(_UW_PLI, vals, uw)
...
def calcular(req:CalcRequest)->CalcResponse:
    ...
    for z in req.zonas:
        ...
        # LC só ativo se risco de explosão/vida imediata (LO > 0 explícito)
        lo_efetivo = z.lo if getattr(z,"tem_lo",False) and z.lo>0 else 0.0
        ...
        for linha in req.linhas:
            ...
            ptu=_PTU.get(linha.ptu,1.0); peb=_PEB.get(linha.peb,1.0)
        ...
        if z.tem_l3:
            ...
            RV3 = sum((L_nl_pld[l.id] + L_ndj_pld[l.id]) * _PEB.get(l.peb, 1.0) * L_cld[l.id] * LB3 for l in req.linhas)
            R3 = RB3+RV3

        # F — Seção 7, Tabela 7
        F=0.0; FB=0.0; FC=0.0; FM=0.0; FV=0.0; FW=0.0; FZ=0.0
        if z.habilitar_f:
            FB = (ND*PB) if z.zpr0a else 0.0
            FC = ND*PC
            # Tabela 7: FM = NM × PM.
            FM = NM * PM
            FV=FW=FZ=0.0
